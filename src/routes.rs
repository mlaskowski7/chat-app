...
pub async fn chat_server(req: HttpRequest, stream: web::Payload, pool: web::Data<DbPool>, srv: web::Data<Addr<server::ChatServer>>) -> Result<HttpResponse, Error> {
    ws::start(
...
#[post("/users/create")]
pub async fn create_user(pool: web::Data<DbPool>, form: web::Json<models::NewUser>) -> Result<HttpResponse, Error> {
    let user = web::block(move || {
        let mut conn = pool.get()?;
        db:: add_new_user(&mut conn, &form.username, &form.phone)
...
pub async fn get_user_by_id(
    ...
    let user = web::block(move || {
        let mut conn = pool.get()?;
        db::find_user_by_uuid(&mut conn, user_id.to_string())
...
pub async fn get_conversation_by_id(
    ...
    if let Ok(Some(data)) = conversations {
